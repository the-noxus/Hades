...
try:
    ...
    print("Discord.py is not installed.\n"
          ...
          "https://twentysix26.github.io/Red-Docs/\n")#Remember to change the website IF making a Self-Bot
    sys.exit(1)
...
#
#       Hades, a bot created by Shadow Scythe, Using IDLE 3.5 for Python 5.3
#
#               https://discord.gg/NJDYddv
#
#       The team that moderates this is the team taht can help you get the bot running,
#
#       

description = "Hades - A multifunctional Bot made to intrest peoples wants and needs"
...
class Bot(commands.Bot):
    ...
    async def shutdown(self, *, restart=False):
        """Gracefully quits Hades with exit code 0
    ...
    async def pip_install(self, name, *, timeout=None):
        """
        Installs the pip package in the local 'lib' folder in a thread safe
...
def initialize(bot_class=Bot, formatter_class=Formatter):
    ...
    async def on_ready():
        ...
        print("-----------------")
        await bot.change_presence(status=discord.Status.online, game=discord.Game(name='<help| <invite | 3 servers'))
        ...
        print("\nOfficial server: https://discord.gg/NJDYddv")
    ...
    async def on_command_error(error, ctx):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            message = ("Error in command '{}'. Check your console or "
                       "logs for details, or message owner."
...
def check_folders():
    folders = ("data", "data/hades", "cogs", "cogs/utils")
...
def interactive_setup(settings):
    ...
    if first_run:
        print("Hades - First run configuration & setup\n")
        print("If you haven't already, create a new account:\n"
              "https://twentysix26.github.io/Red-Docs/red_guide_bot_accounts/ (twentysix26 link, just a side bot)"
    ...
    if first_run:
        ...
        print("\nThe configuration is done. Leave this window always open to"
              ...
              "Please read this guide for a good overview on how Hades works:\n"
...
def set_logger(bot):
    ...
    fhandler = logging.handlers.RotatingFileHandler(
        filename='data/hades/hades.log', encoding='utf-8', mode='a',
    ...
    handler = logging.FileHandler(
        filename='data/hades/discord.log', encoding='utf-8', mode='a')
...
def set_cog(cog, value):  # TODO: move this out of hades.py
    data = dataIO.load_json("data/rhades/cogs.json")
    data[cog] = value
    dataIO.save_json("data/hades/cogs.json", data)
...
def load_cogs(bot):
    ...
    try:
        registry = dataIO.load_json("data/hades/cogs.json")
    except:
        registry = {}
    ...
    if owner_cog is None:
        print("The owner cog is missing. It contains core functions without "
              "which Hades cannot function. Reinstall.")
        exit(1)

    if bot.settings._no_cogs:
        bot.logger.debug("Skipping initial cogs loading (--no-cogs)")
        if not os.path.isfile("data/hades/cogs.json"):
            dataIO.save_json("data/hades/cogs.json", {})
        return
    ...
    dataIO.save_json("data/hades/cogs.json", registry)
