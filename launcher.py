...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="Hades - Discord Bot's launcher")
    parser.add_argument("--start", "-s",
                        help="Starts Hades",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Autorestarts Hades in case of issues",
                        action="store_true")
    parser.add_argument("--update-hades",
                        help="Updates Hades (git)",
                        action="store_true")
...
def update_hades():
    try:
        ...
        return
    if code == 0:
        print("\nHades has been updated")
    else:
        print("\nHades could not update properly. If this is caused by edits "
...
def reset_red(reqs=False, data=False, cogs=False, git_reset=False):
    ...
    if git_reset:
        ...
        if code == 0:
            print("Hades has been restored to the last local commit.")
        else:
...
def download_ffmpeg(bitness):
    clear_screen()
    repo = "https://www.facebook.com/the.101.zone"  #This will have to be changed if making a Self-Bot Red or Hades is the oringinal creators, Self-Bots are only to be created from the Creator no SIGN of creating from other bot.
    verified = []
...
def verify_requirements():
    sys.path_importer_cache = {} # We don't know if the cache reset has any 
...
def requirements_menu():
    ...
    while True:
        ...
        print("1. Install audio +basic requirements (recommended)")
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("Hades:")
        print("1. Update Hades + requirements (recommended)")
        print("2. Update Hades")
...
def maintenance_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            if user_pick_yes_no():
                reset_hades(git_reset=True)
                wait()
        elif choice == "2":
            print("Are you sure? This will wipe the 'data' folder, which "
                  "contains all your settings and cogs' data.\nThe 'cogs' "
                  "folder, however, will be left intact leaving nothing "
                  "but the required 'cogs' to run Hades.")
            if user_pick_yes_no():
                reset_hades(data=True)
                wait()
        elif choice == "3":
            reset_hades(reqs=True)
            wait()
        elif choice == "4":
            print("Are you sure? This will wipe ALL your Bot's installation "
                  "data.\nYou'll lose all your settings, cogs and any "
                  "modification you have made.\nThere is no going back. "
                  "When it gets deleted there is a chance of you having "
                  "left and having to reinstall it over again")
            if user_pick_yes_no():
                reset_hades(reqs=True, data=True, cogs=True, git_reset=True)
                wait()
        elif choice == "0":
            break
        clear_screen()
...
def run_hades(autorestart):
    interpreter = sys.executable

    if interpreter is None: # This should never happen it can lead to huge unfixable problems
        raise RuntimeError("Couldn't find Python's interpreter")

    if verify_requirements() is None:
        print("You don't have the requirements to start Hades. "
              "Install them from the launcher.")
        if not INTERACTIVE_MODE:
            exit(1)

    cmd = (interpreter, "hades.py")
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting Hades...")
                continue
            else:
                if not autorestart:
                    break

    print("Hades has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Hades without going
    ...
    start_hades = "{} --start".format(call)
    start_hades_autorestart = "{} --start --auto-restart".format(call)
    modified = False
    ...
    start_hades             = ccd + start_hades             + pause
    start_hades_autorestart = ccd + start_hades_autorestart + pause

    files = {
        "start_hades"             + ext : start_hades,
        "start_hades_autorestart" + ext : start_hades_autorestart
    }
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Hades Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        if not is_git_installation:
            print("WARNING: It doesn't look like Hades has been "
                  ...
                  "properly this time:\n"
                  "https://facebook.com/the.101.zone\n")
        ...
        print("1. Run Hades /w autorestart in case of issues")
        print("2. Run Hades")
        print("3. Update")
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Hades needs Python 3.5 or superior Version 6+ seems not to work. Install the required " #Python 6+ doesn't work properly for Hades,
              "version.\nPress enter to continue.")                                                    #we cannot help you if you do use 6+
              
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Hades cannot work without the pip module. Please make sure to "
              "install Python without unchecking any option during the setup")
        wait()
        exit(1)
    if args.repair:
        reset_hades(git_reset=True)
    if args.update_hades:
        update_hades()
    if args.update_reqs:
        install_reqs(audio=True)
    ...
    if INTERACTIVE_MODE:
        ...
        print("Starting Hades...")
        run_hades(autorestart=args.auto_restart)
